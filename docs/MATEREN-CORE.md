...
- **v1.3 — 2026-07-06:** Sistema TI adopta [Design System de producto v0.3](MATEREN-DESIGN-SYSTEM.md): verde petróleo `#072E2A` + acento `#157955` (muestreado del logo), estética tipo shadcn en CSS custom. Índigo/Navy placeholder de v1.2 queda obsoleto.
- **v1.2 — 2026-07-06:** Sistema TI implementa tokens `--mat-*` con alias `--color-*` (supersedido por v1.3).
...
|---|---|---|
| Sistema TI | ~~Paleta producto v0.3~~ **Hecho (2026-07-06):** verde petróleo + acento en `main.css`, estilo shadcn (bordes, focus ring, sin sombras). Pendiente: migrar referencias `--color-*` → `--mat-*` gradualmente; portar componentes faltantes del HTML prototipo (tabs, toggle, alert). | — |
