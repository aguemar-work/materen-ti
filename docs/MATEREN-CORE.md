...
|---|---|---|
| Sistema TI | Paleta Cyprus `#004741` + Sand `#f0ede4` → Navy `#11133C` + Turquesa/Índigo de marca, incluyendo las 8 familias semánticas ya construidas (danger 9°, success 100°, warning 36°, info 205°, purple 265°, sky 190°, teal 170°, neutral) — la fórmula HSL se conserva, solo cambia el hue base. Prefijo `--color-*` → `--mat-*` (alias, no reescritura completa de golpe). Referencias internas a "Materen Core §2/§6" (numeración v0.5, ya desactualizada) → anclas de esta versión. | Color: visible al usuario, brecha de mayor prioridad. Prefijo y referencias: sin impacto visual, pero un agente de IA que las siga hoy llega a la sección equivocada. |
