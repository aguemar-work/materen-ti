...
| EMPLEADOS-REINGRESO-DNI-SIN-MANEJO | Dar de alta un empleado con un DNI ya existente (el `unique` de `dni`, migración 002, es global — no distingue activo/inactivo) por el camino normal "Nuevo empleado" revienta con el error crudo de Postgres (`23505 duplicate key`), sin manejo amigable ni oferta de reactivar el registro existente. Solo "Migrar a empleado" desde `/personal` (vía `buscarPorDni`, que sí encuentra la coincidencia) maneja el conflicto abriendo el formulario en modo edición — pero tampoco reactiva `estado` automáticamente: si el empleado encontrado estaba `Inactivo`, sigue `Inactivo` después de "migrar", hay que entrar a la ficha y reactivarlo aparte. Detectado al auditar el flujo completo de Empleados de punta a punta | Baja | **Abierto** (detectado 2026-08-25) | `frontend/src/modules/empleados/EmpleadoForm.vue` (`guardar()`, catch genérico) vs. `frontend/src/modules/personal/PersonalRegistrosView.vue` + `frontend/src/api/domains/empleados.js` (`buscarPorDni`) |
...
| CUENTAS-SIN-AUDITORIA-CRUD | Alta, edición, traspaso, cierre de asignación y revocación de una cuenta (`createCuenta`, `updateCuenta`, `traspasarCuenta`, `cerrarAsignacion`, RPC `revocar_cuenta_personal()`) no dejan ningún rastro en `accesos_log` — solo el revelado (`ver`/`copiar`) y la entrega (`enviar`/`entrega_abierta`) quedan auditados, porque dependen de pasar obligatoriamente por la edge function `credenciales.ts`. El CRUD normal va directo cliente→tabla (`frontend/src/api/domains/cuentas.js`), sin ningún trigger `AFTER INSERT/UPDATE/DELETE` sobre `cuentas`/`asignaciones_cuenta` que audite (los únicos triggers existentes son de notificaciones realtime, no auditoría). Confirmado con datos reales de producción: 197 filas en `cuentas`, 168 en `asignaciones_cuenta`, cero eventos de alta/edición/revocación en `accesos_log`. Contraste directo con Tickets (`ticket_eventos`), Equipos (`eventos_equipo`) y Accesos Sensibles, los 3 con auditoría de ciclo de vida completo vía trigger DB-enforced — Cuentas queda en un punto intermedio, mejor que Empleados (sin auditoría alguna) pero peor que esos 3. Detectado al auditar el flujo completo de Cuentas de punta a punta | Baja | **Abierto** (detectado 2026-08-26) | `frontend/src/api/domains/cuentas.js` (`createCuenta`, `updateCuenta`, `traspasarCuenta`, `cerrarAsignacion`), `migrations/077_revocar_cuenta_personal_atomico.sql` — ningún `insert` a `accesos_log` en ninguno |
| ENTREGACREAR-RATELIMIT-BYPASS | El rate-limit de revelado (`REVELADO_MAX=40`/`REVELADO_VENTANA_MIN=5min` por usuario, `functions/credenciales.ts`) se calcula con `reveladosRecientes()`, que solo suma filas de `accesos_log` con `accion IN ('ver','copiar')`. `entregaCrear` registra cada credencial entregada con `accion:'enviar'` — un valor que ese conteo no incluye — así que las llamadas a `entregaCrear` (hasta `ENTREGA_MAX_CUENTAS=20` cuentas por lote, sin tope de llamadas) nunca hacen avanzar el contador que sí comparten `revelar`/`revelarClaveLicencia`/`revelarAccesoSensible` entre sí. Un usuario con permiso puede encadenar `entregaCrear` repetidamente para revelar credenciales muy por encima del límite pensado, sin activar nunca el bloqueo de 429. Detectado al auditar el flujo completo de Cuentas de punta a punta | Baja | **Abierto** (detectado 2026-08-26) | `functions/credenciales.ts` (`reveladosRecientes`, líneas ~239-248; `entregaCrear`, líneas ~609-680, `accion:'enviar'` en el `log()` de la línea ~683-691) |
| REVOCAR-CUENTA-LOGICA-DUPLICADA | `dar_baja_empleado()` (migración 038, 2026-08-05) y `revocar_cuenta_personal()` (migración 077, 2026-08-21) implementan de forma independiente el mismo concepto de negocio — cerrar la asignación y hacer soft-delete de una cuenta `tipo_cuenta='personal'` — sin que ninguna reutilice a la otra. `dar_baja_empleado()` no fue tocada por la migración 077 (su alcance fue deliberadamente angosto, ver comentario de cabecera) porque ya soft-eliminaba correctamente desde antes; no es un bug de ninguna de las dos migraciones, sino deuda de diseño no documentada. Riesgo: si el concepto de "revocar cuenta personal" cambia en el futuro (nueva validación, nuevo efecto colateral) en una de las dos funciones, la otra puede divergir silenciosamente sin que nada lo fuerce a mantenerse sincronizado. Detectado al auditar el flujo completo de Cuentas de punta a punta | Baja | **Abierto** (detectado 2026-08-26) | `migrations/038_baja_empleado_atomica.sql` (líneas 37-92) vs. `migrations/077_revocar_cuenta_personal_atomico.sql` (líneas 30-66) |
| ENTREGAS-JEFE-PUEDE-BORRAR | La policy RLS `"solo jefe puede eliminar entregas"` (`migrations/010_seguridad.sql`, sin cambios desde entonces) permite a cualquier JEFE eliminar físicamente una fila de `entregas` desde el cliente — borrando el registro de que esa entrega de credenciales existió alguna vez (`empleado_nombre`, `expires_at`, `viewed_at`, `token_hash`). El rastro real e inmutable sobrevive en `accesos_log` (`enviar`/`entrega_abierta`), donde nadie —ni JEFE— tiene permiso de escritura desde cliente, así que la entrega en sí misma no es un registro de auditoría a prueba de manipulación, a diferencia de `ticket_eventos`/`eventos_equipo` (sin ninguna policy de escritura, ni para JEFE). Detectado al auditar el flujo completo de Cuentas de punta a punta | Baja | **Abierto** (detectado 2026-08-26) | `migrations/010_seguridad.sql` (policy DELETE de `entregas`) |
| CREDENCIALES-REVELADO-DENEGADO-SIN-AUDITAR | Cuando `revelar` (o `entregaCrear`) rechaza un intento por falta de permiso (`tienePermisoCredenciales`/`tienePermisoModulo`/regla de `personal` solo-JEFE, `functions/credenciales.ts`), el `return json({ok:false...}, 403)` correspondiente no pasa por `log()` — solo se auditan los revelados exitosos, las entregas fallidas (`entrega_fallida`, token inválido/expirado) y los bloqueos de ruta por rol (`acceso_denegado`, guard del router). Un intento real de un ASISTENTE tratando de revelar sin tener `credenciales.ver`, o de revelar una cuenta `personal` sin ser JEFE, no deja ningún rastro en `accesos_log`. Detectado al auditar el flujo completo de Cuentas de punta a punta | Baja | **Abierto** (detectado 2026-08-26) | `functions/credenciales.ts` (líneas ~528-544, los `return` de rechazo de `revelar` sin llamar a `log()`) |
| TICKETS-TABLAS-SATELITE-SIN-GATE | La migración 072 instaló `es_jefe() OR (es_staff() AND tiene_permiso_modulo('tickets'))` en `tickets` (SELECT/UPDATE) con el objetivo declarado de cerrar el hueco de módulo en el dominio Tickets — pero nunca tocó 5 tablas satélite del mismo dominio: `ticket_comentarios`, `ticket_eventos`, `ticket_satisfaccion`, `subcategorias_ticket`, `transiciones_ticket_permitidas`. Tercer caso del mismo patrón exacto: EQUIPOS-TABLAS-SATELITE-SIN-GATE (migración 079) y CUENTAS-TABLAS-SATELITE-SIN-GATE (migración 081) — esta vez detectado no dominio por dominio, sino por un barrido general de todo el esquema `public` buscando cualquier tabla con `es_staff()` plano cuya tabla principal ya estuviera gateada por módulo. Confirmado por `SELECT` directo a `pg_policies` (no por el nombre de la migración): las 5 seguían con `es_staff()` plano en los comandos que no eran DELETE jefe-only (`ticket_comentarios`: INSERT/SELECT; `ticket_eventos`/`ticket_satisfaccion`/`transiciones_ticket_permitidas`: solo SELECT, sin policy de INSERT/UPDATE/DELETE para el cliente — append-only vía trigger o whitelist de solo lectura; `subcategorias_ticket`: SELECT/INSERT/UPDATE, DELETE ya `es_jefe()`). El de mayor sensibilidad del lote es `ticket_comentarios`, que incluye comentarios internos (`interno=true`) no destinados al solicitante — un ASISTENTE sin el módulo `tickets` podía leerlos y escribir comentarios nuevos directo por SDK. El barrido también confirmó 2 hallazgos ambiguos sin fix mecánico (`categorias_ticket`, compartida con Base de Conocimiento; `ubicaciones`, compartida con Empleados) y que `areas_obras` (Empleados) tiene el mismo problema — pendientes de decisión y migración aparte, no incluidos acá. Detectado por el barrido general del patrón "tabla satélite sin gate de módulo" en todo el esquema | Media-Alta (acceso no autorizado real vía SDK directo — requiere ser staff activo, no explotable de forma anónima ni remota trivial; mismo criterio de severidad que los 2 hallazgos gemelos) | **Resuelto** (2026-08-26) | Migración 082 (`082_rls_tickets_tablas_satelite.sql`) reemplaza las 8 policies que usaban `es_staff()` plano por el mismo patrón de 072 (2 en `ticket_comentarios`: INSERT/SELECT; 1 en `ticket_eventos`: SELECT; 1 en `ticket_satisfaccion`: SELECT; 3 en `subcategorias_ticket`: SELECT/INSERT/UPDATE; 1 en `transiciones_ticket_permitidas`: SELECT); el DELETE de `subcategorias_ticket` (ya `es_jefe()`) queda sin tocar. Verificación en dos capas antes de aplicar a producción, mismo método aceptado desde los 2 hallazgos gemelos: (1) evaluación directa por `SELECT` de `es_jefe()`/`es_staff()`/`tiene_permiso_modulo('tickets')` contra los datos reales de las cuentas fixture en un branch de InsForge descartable — confirmó que `ci-tests-sin-modulo` (ASISTENTE activo sin el módulo `tickets`) pasaba la policy vieja y no la nueva, que `ci-tests@materen-ti-test.local` (ASISTENTE que ya tiene el módulo `tickets` otorgado) pasa igual en ambas versiones sin necesidad de otorgar nada a mano, y que ambas cuentas JEFE pasan igual en las dos versiones (sin regresión); (2) revisión de los flujos reales que leen estas tablas: las 4 con UI (`ticket_comentarios`/`ticket_eventos`/`ticket_satisfaccion`/`subcategorias_ticket`) se consumen exclusivamente desde `frontend/src/api/domains/tickets.js` (archivo marcado explícitamente "Dominio tickets (staff)" — la creación pública, el seguimiento y la encuesta viven aparte en `ticketsPublicos.js` vía edge function, sin RLS directo sobre estas tablas), alcanzables solo desde rutas ya gateadas por `modulo:'tickets'`; `transiciones_ticket_permitidas` no tiene ningún consumidor de cliente en absoluto (solo se lee server-side desde el trigger de validación de transiciones, que corre con permisos de tabla, no de RLS de cliente) — cambio sin impacto de flujo en ningún caso. Aplicado a producción y verificado byte a byte contra `pg_policies` (9/9 policies idénticas al branch probado), registrado en `schema_migrations` (`aplicada_por='INACONS'`), branch descartable borrado |
| PROBLEMAS-TABLAS-SATELITE-SIN-GATE | La migración 072 instaló `es_jefe() OR (es_staff() AND tiene_permiso_modulo('problemas'))` en `problemas` (SELECT/INSERT/UPDATE) con el objetivo declarado de cerrar el hueco de módulo en el dominio Problemas — pero nunca tocó 2 tablas satélite del mismo dominio: `acciones_correctivas` y `problema_tickets`. Cuarto caso del mismo patrón exacto: EQUIPOS-TABLAS-SATELITE-SIN-GATE (migración 079), CUENTAS-TABLAS-SATELITE-SIN-GATE (migración 081) y TICKETS-TABLAS-SATELITE-SIN-GATE (migración 082) — encontrado por el mismo barrido general de todo el esquema `public` que encontró los otros 3. Confirmado por `SELECT` directo a `pg_policies` (no por el nombre de la migración): las 2 seguían con `es_staff()` plano en los comandos que no eran DELETE jefe-only (`acciones_correctivas`: SELECT/INSERT/UPDATE, DELETE ya `es_jefe()`; `problema_tickets`: SELECT/INSERT/DELETE, sin policy de UPDATE — es tabla de vínculo N a N, se crea o se borra la fila, no se edita). A diferencia de los 2 hallazgos gemelos anteriores, acá el DELETE de `problema_tickets` también pasó al gate de módulo en vez de quedar jefe-only — nunca lo estuvo, y desvincular un ticket de un problema es una operación normal del módulo, mismo criterio que `equipo_accesorios`/`equipos_importacion` en la migración 079. Un ASISTENTE sin el módulo `problemas` podía leer/crear/editar acciones correctivas de cualquier problema, y leer/vincular/desvincular tickets de cualquier problema, directo por SDK. Detectado por el barrido general del patrón "tabla satélite sin gate de módulo" en todo el esquema | Media-Alta (acceso no autorizado real vía SDK directo — requiere ser staff activo, no explotable de forma anónima ni remota trivial; mismo criterio de severidad que los 3 hallazgos gemelos) | **Resuelto** (2026-08-26) | Migración 083 (`083_rls_problemas_tablas_satelite.sql`) reemplaza las 6 policies que usaban `es_staff()` plano por el mismo patrón de 072 (3 en `acciones_correctivas`: SELECT/INSERT/UPDATE; 3 en `problema_tickets`: SELECT/INSERT/DELETE); el DELETE de `acciones_correctivas` (ya `es_jefe()`) queda sin tocar. Verificación en dos capas antes de aplicar a producción, mismo método aceptado desde los 3 hallazgos gemelos: (1) evaluación directa por `SELECT` de `es_jefe()`/`es_staff()`/`tiene_permiso_modulo('problemas')` contra los datos reales de las cuentas fixture en un branch de InsForge descartable — confirmó que `ci-tests-sin-modulo` y `ci-tests@materen-ti-test.local` (ambos ASISTENTE activo sin el módulo `problemas`) pasaban la policy vieja y no la nueva; como ninguna cuenta ASISTENTE fixture tenía el módulo `problemas` otorgado, se le otorgó temporalmente a `ci-tests-sin-modulo` en el branch para confirmar el caso positivo — pasa la policy nueva, sin sobre-restringir; ambas cuentas JEFE pasan igual en las dos versiones (sin regresión); (2) revisión de los flujos reales: `acciones_correctivas`/`problema_tickets` se consumen exclusivamente desde `frontend/src/api/domains/problemas.js`, alcanzables solo desde rutas ya gateadas por `modulo:'problemas'` — sin impacto en el detalle de Problema (acciones correctivas listadas/creadas/editadas) ni en vincular/desvincular tickets. Aplicado a producción y verificado byte a byte contra `pg_policies` (7/7 policies idénticas al branch probado), registrado en `schema_migrations` (`aplicada_por='INACONS'`), branch descartable borrado |
...
9. `MAX_FOTOS=4` sin tope server-side en `equipos-fotos.ts`.
...
20. `personal-registro.buscarDni` no excluye ex-empleados (`deleted_at`),
    a diferencia de Tickets — asimetría de comportamiento entre los 2
    endpoints públicos que resuelven por DNI, no de seguridad. Ver
...
25. `dar_baja_empleado()` y `revocar_cuenta_personal()` implementan la
    ...
    futuro. Ver REVOCAR-CUENTA-LOGICA-DUPLICADA (Ciclo 13).
26. Un JEFE puede eliminar físicamente una fila de `entregas`, borrando
    el registro de que existió (el rastro real en `accesos_log`
    sobrevive, nadie tiene permiso de escritura sobre esa tabla desde
    cliente). Ver ENTREGAS-JEFE-PUEDE-BORRAR (Ciclo 13).
27. Los intentos de revelado de credenciales rechazados por falta de
